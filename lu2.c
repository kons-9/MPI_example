...
void MyLUsolve(double *localA, double *b, double x[N], int n);
...
void MyLUsolve(double *localA, double *b, double *x, int n) {
  // localA: (n, ib)
  // b: (n, 1)
  // x: (n, 1)
  // n: matrix size
  ...
  for (int k = 0; k < iend; k++) {
    int k_proc = (k / ib);
    if (myid == k_proc) {
      double dtemp = 1.0 / localA[k * ib + k - istart];
      for (int i = k + 1; i < n; i++) {
        localA[i * ib + k - istart] = localA[i * ib + k - istart] * dtemp;
        buf[i] = localA[i * ib + k - istart];
      }
    ...
    for (int col = k + 1; col < n; col++) {
      ...
      for (int row = tstart - istart; row < ib; row++) {
        // localA[col][row] = localA[col][row] - localA[k][row] * dtemp;
        localA[col * ib + row] =
            localA[col * ib + row] - localA[k * ib + row] * dtemp;
      }
    }
  }
    ...
    if (block / ib == myid) {
      for (int i = iend - 1; i >= istart; i--) {
        ...
        for (int j = iend - 1; j > i; j--) {
          x[i] -= localA[i * ib + j - istart] * x[j];
        }
        x[i] = x[i] / localA[i * ib + i - istart];
      }
    } else {
